...
This chapter can even link back to other whole chapters @sec-binary-response. Or equations in other chapters @eq-AROC. This is only true for the book template.
...
New prefixes can be created, as this template does below to create new figures for videos called `vid`. Various default ones already exist like Theorems, Lemmas etc.. this template will demo the `numbered-custom-boxes` in @sec-niceboxes as an alternative. This alternative gives fine-grained numbering control, so you'll need to avoid Quarto's numbering. So if you do follow this recommendation then you must avoid using standard Quarto names like `#thm-fermat` because Quarto will be confused about which method you wish. So in the examples presented in @sec-niceboxes I have just started all references with `#r` to avoid overlap, e.g. `#rthm-fermat`.
...
Personally I prefer to hide the videos from direct view to keep the window clean, so you'll see an alternative method in @sec-niceboxes, where I put them inside folded boxes.
