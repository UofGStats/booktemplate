...
This code has then been added to the `_quarto.yml` or `_metadata.yml` file (or at the top of this file) to load the coloured boxes.
...
Each block is foldable (open by default) and has its own color scheme and group style as defined in your YAML configuration.

Notice Theorem, Corollary and Lemma has been defined to share counter. But Definition and Video have their own types (*deflike* and *vidlike*).
...
Each box has to define four colours. In the file mentioned above the light-mode border and background are set. For those using dark-mode you also need to define a dark mode pair. See [dark box colour stylesheet](themes/dark-styles-boxes.scss).
